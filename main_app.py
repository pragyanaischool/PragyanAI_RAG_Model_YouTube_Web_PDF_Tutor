...
Features native yt-dlp discovery, instant video preview with on-demand Whisper transcription,
Deep-Dive Markdown Notes Studio with PDF/MD export, Exam Solver with Citations, and Multilingual Voice RAG.
"""
...
def notes_to_str(content) -> str:
    """Safely converts list or non-string notes content into a unified markdown string."""
...
with st.sidebar:
    ...
    if st.button("🚀 Ingest & Generate Notes"):
        new_docs = []
...
with tab1:
    ...
    if st.session_state["yt_search_results"]:
        ...
        for i, vid in enumerate(st.session_state["yt_search_results"]):
            with cols[i % 3]:
                ...
                # Step 1: Select and render immediately in player
    ...
    # Step 2: Display Video Player & On-Demand Transcribe Action
    if st.session_state["selected_video"]:
        ...
        with col_player:
            ...
            # Action Button to Extract Audio, Transcribe, and Add to Vector DB
...
with tab2:
    ...
    with sub1:
        if not st.session_state["doc_notes"]:
            st.info("Ingest sources from the sidebar or transcribe a YouTube video to view deep-dive notes.")
        else:

...
Features arXiv Research Paper, Wikipedia, Blog, and Google Link Discovery,
YouTube Studio, Deep-Dive Notes Studio, Exam Solver with Citations, and Multilingual Voice RAG.
"""
...
# Cross-version chain import with fallback to pure LCEL
try:
...
def notes_to_str(content) -> str:
    """Safely converts list or non-string notes content into unified markdown string."""
    ...
    if st.button("🚀 Ingest Files & Generate Notes"):
        ...
        with st.spinner("Extracting sources, vectorizing, and expanding deep-dive notes..."):
            ...
            if new_docs:
                ...
                st.success(f"✅ Ingested {len(new_docs)} source(s) and synthesized study notes!")
            else:
...
with tab1:
    ...
    with c_query:
        search_topic = st.text_input(
            "Search research topic or concept:",
            placeholder="e.g. Diffusion Models, Quantum Computing, Attention Mechanisms",
    ...
    with c_btn:
        st.write("")
        trigger_search = st.button("🔎 Search Sources")

    if trigger_search and search_topic:
        with st.spinner("Searching multi-source repositories..."):
            aggregated_results: list = []

            if source_category in ["All Sources", "arXiv Research Papers"]:
                arxiv_hits = search_arxiv_papers(search_topic, max_results=4)
                aggregated_results.extend(arxiv_hits)

            if source_category in ["All Sources", "Wikipedia Pages"]:
                wiki_hits = search_wikipedia_articles(search_topic, max_results=3)
                aggregated_results.extend(wiki_hits)

            if source_category in ["All Sources", "Tech Blogs & Medium"]:
                blog_hits = search_google_and_blogs(search_topic, target_type="blogs", max_results=3)
                aggregated_results.extend(blog_hits)

            if source_category in ["All Sources", "Articles & Guides"]:
                article_hits = search_google_and_blogs(search_topic, target_type="articles", max_results=3)
                aggregated_results.extend(article_hits)

            if source_category in ["All Sources", "Google Search Links"]:
                google_hits = search_google_and_blogs(search_topic, target_type="all", max_results=4)
                aggregated_results.extend(google_hits)
                    ...
                    if st.button("📖 Explore & Ingest", key=f"acad_explore_{idx}"):
                        with st.spinner(f"Ingesting {item['title'][:25]} & synthesizing notes..."):
                            ...
                            if item.get("content"):
                                ...
                                extracted_web = extract_from_url(item["url"])
                                if extracted_web and extracted_web.get("content"):
                                    payload = extracted_web
                                else:
                                    payload = {
                                        "source": item["url"],
                                        "type": "web",
                            ...
                            st.session_state["extracted_docs"].append(payload)
                            st.session_state["vectorstore"] = index_documents_to_chroma(
                                st.session_state["extracted_docs"]
                            )

                            expanded_notes = generate_single_doc_notes(
                                payload["title"],
                                payload["type"],
                                payload["content"],
                            )
                            st.session_state["doc_notes"][payload["title"]] = expanded_notes
                            st.session_state["master_notes"] = generate_combined_master_notes(
                                st.session_state["extracted_docs"]
                            )
                            st.success(f"✅ Ingested, expanded notes, and indexed into Vector DB!")
            ...
            if st.button("🎧 Extract Audio, Transcribe & Index to Vector DB", type="primary"):
                with st.spinner("Extracting audio, running Groq Whisper & updating Vector DB..."):
                    ...
                    if trans_data:
                        ...
                        st.session_state["trans_text"] = translate_content(trans_data["content"], lang_code)
                        st.session_state["extracted_docs"].append(trans_data)
                        st.session_state["vectorstore"] = index_documents_to_chroma(st.session_state["extracted_docs"])
                        st.session_state["doc_notes"][trans_data["title"]] = generate_single_doc_notes(
                            trans_data["title"], trans_data["type"], trans_data["content"]
                        )
                        st.success("✅ Transcribed and successfully indexed into Vector DB!")
                        st.rerun()
                    else:
                        st.error("❌ Failed to transcribe video audio. Please try another video.")
...
with tab3:
    ...
    with sub1:
        if not st.session_state["doc_notes"]:
            st.info("Ingest sources via arXiv, Wikipedia, Google Search, YouTube, or the sidebar to view deep-dive notes.")
        else:
...
with tab5:
    ...
    if active_query:
        if "vectorstore" not in st.session_state:
            st.warning("Please ingest data sources first.")
        else:
